...
export function registerCodegraphStatusCommand(pi: ExtensionAPI): void {
  pi.registerCommand("codegraph-status", {
    ...
    handler: async (_args, ctx) => {
      if (!ctx.hasUI) {
        try {
          const output = await execCodegraph(
            pi,
            ["status", ctx.cwd],
            TIMEOUTS.quick,
          );
          ...
          showCommandOutput(
            ...
            ctx,
            "status",
            `CodeGraph: ${(err as Error).message}`,
            "error",
          );
        }
        return;
...
export function registerCodegraphInitCommand(pi: ExtensionAPI): void {
  pi.registerCommand("codegraph-init", {
    ...
    handler: async (_args, ctx) => {
      if (!ctx.hasUI) {
        ctx.ui.notify?.(
          "Run codegraph init in the project directory to initialize.",
          "info",
        );
        return;
      }
      ...
      try {
        ...
        if (doIndex) {
          ...
          ctx.ui.notify(
            "Run /codegraph-index later to build the index.",
            "info",
          );
        }
      } catch (err) {
...
export function registerCodegraphIndexCommand(pi: ExtensionAPI): void {
  pi.registerCommand("codegraph-index", {
    ...
    handler: async (_args, ctx) => {
      if (!ctx.hasUI) {
        ctx.ui.notify?.(
          "Run codegraph index in the project directory to build the index.",
          "info",
        );
        return;
      }
...
export function registerCodegraphSyncCommand(pi: ExtensionAPI): void {
  pi.registerCommand("codegraph-sync", {
    description:
      "Incrementally sync the CodeGraph index for the current project",
    handler: async (_args, ctx) => {
      const initialized = await hasCodegraph(ctx.cwd);
      if (!initialized) {
        showCommandOutput(
      ...
      try {
        ...
        showCommandOutput(
          ...
          ctx,
          "sync",
          output || "CodeGraph sync completed.",
          "info",
        );
      } catch (err) {
...
export function registerSessionStartStatus(pi: ExtensionAPI): void {
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    const _unsub = pi.on("agent_start", async () => {
      if (notified) return;
      notified = true;
